...
### Recorder updates for sampled measurements
...
Configured-circuit room humidity and heat-pump power-request percentage are
also sampled measurements and use `force_update`.
...
false. Heat-pump flow-rate values are holding/configuration values, not sampled
measurements, and remain false as well.
[`test_entities.py`](../../tests/test_entities.py) verifies representative
temperature, humidity, power-request, energy, and effective-setpoint behavior;
the selection is owned by
[`sensor.py`](../../custom_components/hacs_wab11/sensor.py),
